...
export default () => {
  passport.use(
    new BasicStrategy(async (username, password, done) => {
      try {
        ...
        clientRes.findByClientId(username).then(client => {
  ...
  passport.use(
    new ClientPasswordStrategy((clientId, clientSecret, done) => {
      try {
        const clientRes = new ClientRepository();
        clientRes.findById(clientId).then(client => {
          if (!client) return done(null, false);
  ...
  passport.use(
    new BearerStrategy(async (token, done) => {
      try {
        ...
        accessTokenRes.findByToken(token).then(accessToken => {
          ...
          userRes.findById(accessToken.user_id).then(user => {
