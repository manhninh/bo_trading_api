...
const getUserBalance = async (row) => {
      ...
      if (row.trc20 !== undefined && row.trc20) {
        ...
        if (tronWallet !== undefined && typeof tronWallet == 'object') {
          ...
          if (trc20AccountBalanceOrigin >= Number(config.TRON_TRC20_DEPOSIT_MIN_AMOUNT)) {
            ...
            await walletModel.updateByUserId(row.user_id, { amount_trc20_wallet: Number(trc20AccountBalanceOrigin) });
        ...
        await createUSDTTRC20(user);
      }
      // END: Check for TRC20
      await delay(1000);
      resolve(true);
    } catch (err) {
      console.log('TRC ERROR: ', err.message);
      await delay(1000);
      resolve(true);
    }
  });
